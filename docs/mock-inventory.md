...
| 인증된 SSE 이벤트 | C5의 `server/frontend/src/services/api/anomaly-stream.api.ts`, `server/frontend/src/features/analysis/hooks/useRealtimeAnomalies.ts`, `server/frontend/src/features/analysis/components/RealtimeAnomalyPanel.tsx`와 backend `schemaVersion: 1`, `provenance: backend-ingest` 계약 | `server/contracts/anomaly.md`의 raw+risk 이벤트. 패널은 KSJ 상세 목업과 분리되어 계약 필드만 표시하며 Bearer·fragmented frame·optional field·invalid/missing token·abort stream QA를 통과 | 이미 실제 API 연결인 항목. 남은 detector/collector 연동과 이벤트 유실 복구 필요성을 별도 결정. 검증 범위는 validation.md 참조 |
