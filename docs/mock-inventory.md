...
현재 구현은 인증 API를 사용하고, 아직 endpoint 계약이 없는 화면에는 목업을 명시한다. API 오류·빈 응답을 목업 성공으로 바꾸지 않는다. 아래는 현재 코드에서 확인한 위치와 교체 조건이며, 원시 샘플 행과 `server/backend/mock_logs.csv` 본문은 문서에 복사하지 않는다.
...
| 분석 목록·상세·상태 | `server/frontend/src/testing/mocks/mockAnalysis.ts`의 `mockAnomalyDetails`와 `server/frontend/src/services/mock/anomaly.mock.ts`; `useAnomalyDetails`·`useAnalysisWorkspace`가 사용 | 인증된 목록/상세/상태 API. `server/frontend/src/services/api/anomaly.api.ts`는 현재 raw log risk 평가(`/api/anomaly/evaluate-risk`)만 제공하고 목록·상세·상태 계약은 없음 | 실제 목록·상세·상태 응답이 동일한 화면 형상을 제공하고 오류를 목업으로 대체하지 않을 때 |
...
| 목업 표시 | `server/frontend/src/components/ui/MockDataNotice.tsx`와 홈·분석·설정·관리자 화면의 안내 배너 | 각 기능의 실제 API 계약과 화면별 데이터 변환 | 해당 화면이 실제 응답만 표시하고 목업 안내가 더 이상 필요하지 않을 때 |
...
| 인증된 SSE 이벤트 | C5의 `server/frontend/src/services/api/anomaly-stream.api.ts`, `server/frontend/src/features/analysis/hooks/useRealtimeAnomalies.ts`, `server/frontend/src/features/analysis/components/RealtimeAnomalyPanel.tsx`와 backend `schemaVersion: 1`, `provenance: backend-ingest` 계약 | `server/contracts/anomaly.md`의 raw+risk 이벤트. 패널은 KSJ 상세 목업과 분리되어 계약 필드만 표시하며 Bearer·fragmented frame·optional field·invalid/missing token·abort stream QA를 통과 | renderer 없는 React hook lifecycle 직접 자동화만 source review로 제한되었고, renderer 확보 시 재연결·중복 제거·로그아웃·unmount 동작을 추가 검증할 수 있음 |
